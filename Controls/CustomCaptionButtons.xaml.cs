...
public sealed partial class CustomCaptionButtons : UserControl, IDisposable
{
    ...
    public void RefreshForTheme(ElementTheme theme)
    {
        ...
        // 新增：根据当前主题设定禁用状态下的图标颜色 (透明度设为 0x66，即 40%)
    ...
    {
        return IsCustomMinButtonEnabled &&
    ...
    {
        return IsCustomMaxButtonEnabled &&
               _appWindow?.Presenter is OverlappedPresenter presenter &&
    ...
    {
        // 直接返回当前主题同步下发的禁用颜色，
        // 避免从 Application.Current 中获取导致与局部 RequestedTheme 状态不同步。
    ...
    private void UpdateClientPressMove(ClientButtonKind kind, FrameworkElement element, PointerRoutedEventArgs e)
    {
        if (_capturedClientButton != kind)
            return;

        SetClientState(kind, IsPointerInside(element, e) ? VisualStateKind.Pressed : VisualStateKind.Normal);
        e.Handled = true;
    }
    ...
    {
        if (_appWindow?.Presenter is OverlappedPresenter presenter && presenter.IsMinimizable)
        {
            presenter.Minimize();
        }
    }
    ...
    private void MinButtonHost_PointerEntered(object sender, PointerRoutedEventArgs e)
    {
        ...
        {
            SetClientState(ClientButtonKind.Minimize, VisualStateKind.Hover);
        }
    }
    ...
    private void MinButtonHost_PointerMoved(object sender, PointerRoutedEventArgs e)
    {
        ...
        ClearMaxHoverFromClientSide();
        UpdateClientPressMove(ClientButtonKind.Minimize, MinButtonHost, e);
    }
    ...
    private void CloseButtonHost_PointerEntered(object sender, PointerRoutedEventArgs e)
    {
        ...
        {
            SetClientState(ClientButtonKind.Close, VisualStateKind.Hover);
        }
    }
    ...
    private void CloseButtonHost_PointerMoved(object sender, PointerRoutedEventArgs e)
    {
        ...
        ClearMaxHoverFromClientSide();
        UpdateClientPressMove(ClientButtonKind.Close, CloseButtonHost, e);
    }
    ...
    private bool TryGetPixelRect(FrameworkElement element, out RectInt32 rect)
    {
        ...
        try
        {
            ...
            var dipRect = transform.TransformBounds(new Windows.Foundation.Rect(0, 0, element.ActualWidth, element.ActualHeight));
